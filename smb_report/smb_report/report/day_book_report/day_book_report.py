...
def get_data(filters):
	data = []
	sinv_data = get_sales_invoice(filters)
	data.extend(sinv_data)
	pinv_data = get_purchase_invoice(filters)
	data.extend(pinv_data)
	pe_data = get_payment_entry(filters)
	data.extend(pe_data)
...
def get_purchase_invoice(filters):
	...
	for inv in inv_details:
		...
		row = [inv.posting_date,invoice_details.doctype,inv.supplier,link,inv.grand_total,inv.net_total,inv.paid_amount,inv.outstanding_amount,inv.mode_of_payment,inv.owner,'',inv.creation]
		data.append(row)
	return data
